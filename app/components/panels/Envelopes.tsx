...
function AdsrControls({
  ...
  return (
    <div className="panel__sliders">
      <Slider label="A" min={0.001} max={5} log value={eg.attackS} format={fmtSeconds} onChange={(attackS) => set({ attackS })} />
      <Slider label="D" min={0.001} max={5} log value={eg.decayS} format={fmtSeconds} onChange={(decayS) => set({ decayS })} />
      <Slider label="S" min={0} max={1} value={eg.sustain} format={fmtPercent} onChange={(sustain) => set({ sustain })} />
      <Slider label="R" min={0.001} max={8} log value={eg.releaseS} format={fmtSeconds} onChange={(releaseS) => set({ releaseS })} />
    </div>
  );
}
